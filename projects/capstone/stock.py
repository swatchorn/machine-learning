...
class ATT():
    ...
    # Include weekends on prices display
    def resample(self, dataframe):
        ...
        # Reset the index and interpolate values
    ...
    # Plot calculated buyinging and holding shares profit for specified date range
    def buy_and_hold(self, start_date=None, end_date=None, nshares=1):
        ...
        print('{} Total buy & hold profit from {} to {} for {} shares = ${:.2f}'.format
              (self.symbol, start_date, end_date, nshares, total_hold_profit))
        
        # Plot the total profits 
        plt.style.use('seaborn-talk')
        
        # Location for profit amount or number
        text_location = (end_date - pd.DateOffset(months = 1))
        
        # Plot the profits over time
        plt.plot(profits['Date'], profits['hold_profit'], 'b', linewidth = 1)
        plt.ylabel('Profit ($)'); plt.xlabel('Date'); plt.title('Buy & Hold Profits for {} {} to {}'.format(
    ...
    def changepoint_prior_analysis(self, changepoint_priors=[0.001, 0.05, 0.1, 0.2], colors=['b', 'r', 'gold', 'lightgreen']):
        ...
        ax.plot(train['ds'], train['y'], 'bo', ms = 4, label = 'Training Observations')
        color_dict = {prior: color for prior, color in zip(changepoint_priors, colors)}

        # Plot ea changepoint predictions
        for prior in changepoint_priors:
            # Plot the predictions
            ax.plot(predictions['ds'], predictions['%.3f_yhat' % prior], linewidth = 1,
    ...
    # Basic prophet model for selected number of days  
    def create_prophet_model(self, days=0, resample=False):
        ...
        # Predict for next year with future dataframe
        ...
        if days > 0:
            ...
            title = '%s Historical & Predicted Stock Price'  % self.symbol
        else:
            title = '%s Historical & Modeled Stock Price' % self.symbol
        
        # Set up the plot
        fig, ax = plt.subplots(1, 1)

        # Plot the actual values
        ax.plot(stock_history['ds'], stock_history['y'], 'ko-', linewidth = 1, alpha = 0.8, ms = 1.8, label = 'Actual')
        
        # Plot the predicted values
        ax.plot(future['ds'], future['yhat'], 'blue',linewidth = 1.5, label = 'Modeled');

        # Plot the uncertainty interval ribbon
        ax.fill_between(future['ds'].dt.to_pydatetime(), future['yhat_upper'], future['yhat_lower'], alpha = 0.3, 
                       facecolor = 'yellow', edgecolor = 'k', linewidth = 1, label = 'Confidence Interval')
    ...
    def evaluate_prediction(self, start_date=None, end_date=None, nshares = None):
        ...
        # Future dataframe and predictions
        future = model.make_future_dataframe(periods = 365, freq='D')
        future = model.predict(future)
        
        # Predictions with the known values
        ...
        if not nshares:
            ...
            print('Av. Absolute Error on Training Data = ${:.2f}.'.format(train_mean_error))
            print('Av. Absolute Error on Testing  Data = ${:.2f}.\n'.format(test_mean_error))

            # Direction accuracy
            print('The price increased when predicted an increase {:.2f}% of the time.'.format(increase_accuracy))
            print('The price decreased when predicted a decrease  {:.2f}% of the time.\n'.format(decrease_accuracy))
            ...
            # Plot the actual values
            ax.plot(train['ds'], train['y'], 'bo-', linewidth = 0.8, alpha = 0.8, ms = 1.8, label = 'Training Observations')
            ax.plot(test['ds'], test['y'], 'yo-', linewidth = 0.8, alpha = 0.8, ms = 1.8, label = 'Testing Observations')
            
            # Plot the predicted values
            ax.plot(future['ds'], future['yhat'], 'r', linewidth = 1, label = 'Predicted');

            # Plot the uncertainty interval as ribbon
            ax.fill_between(future['ds'].dt.to_pydatetime(), future['yhat_upper'], future['yhat_lower'], alpha = 0.6, 
                           facecolor = 'lightcyan', edgecolor = 'lightgrey', linewidth = 1.4, label = 'Confidence Interval')

            # Put a vertical line at the start of predictions
            plt.vlines(x=min(test['ds']), ymin=min(future['yhat_lower']), ymax=max(future['yhat_upper']), colors = 'g',
        ...
        # Test the marker using shares
            ...
            for i, correct in enumerate(test_pred_increase['correct']):
                ...
                # positive gain from prediction
                if correct == 1:
                    prediction_profit.append(nshares * test_pred_increase.loc[i, 'real_diff'])
                # negative gain from prediction
                else:
                    prediction_profit.append(nshares * test_pred_increase.loc[i, 'real_diff'])
            
            test_pred_increase['pred_profit'] = prediction_profit
            
            # Put profit into test dataframe
            ...
            # Display profit info
            print('Total profit using the Prophet model = ${:.2f}.'.format(np.sum(prediction_profit)))
            print('Buy & Hold strategy profit =         ${:.2f}.'.format(float(test.loc[test.index[-1], 'hold_profit'])))
            ...
            plt.title('Predictive v. Buy & Hold Profits');
            plt.legend(loc = 2, prop={'size': 10});
            plt.grid(alpha=0.2); 
            plt.show()
     ...
     def changepoint_date_analysis(self, search=None):
        ...
        # Sort max change values
        c_data = c_data.sort_values(by='abs_delta', ascending=False)

        # 10 largest changepoints
        c_data = c_data[:10]

        # Neg and pos changepoints
        ...
        if not search:
            ...
            # Plot actual values, estimated values, and changepoints
            self.reset_plot()
            
            # Set up line plot 
            plt.plot(train['ds'], train['y'], 'yo', ms = 4, label = 'Stock Price')
            plt.plot(future['ds'], future['yhat'], color = 'blue', linewidth = 1, label = 'Modeled')
        ...
        if search:
            ...
            # Get the Google Trends and join to training dataframe
            ...
            # Plot the normalized stock price and search frequency
    ...
    def predict_future(self, days=30):
        ...
        ax.errorbar(future['Date'].dt.to_pydatetime(), future['estimate'], 
                   ...
                   ecolor='dodgerblue', capsize = 4, elinewidth = 1, label = 'Pred with Range')
    ...
    def changepoint_prior_validation(self, start_date=None, end_date=None,changepoint_priors = [0.001, 0.05, 0.1, 0.2]):
        ...
        plt.plot(results['cps'], results['train_err'], 'ro-', ms = 8, label = 'Train Error')
        plt.plot(results['cps'], results['test_err'], 'b*-', ms = 8, label = 'Test Error')
        plt.xlabel('Changepoint Prior Scale (CPS)'); plt.ylabel('Avg. Absolute Error ($)');
        plt.title('Training & Testing Curves Relative to CPS')
        ...
        plt.plot(results['cps'], results['train_range'], 'ro-', ms = 8, label = 'Train Range')
        plt.plot(results['cps'], results['test_range'], 'b*-', ms = 8, label = 'Test Range')
        plt.xlabel('Changepoint Prior Scale (CPS)'); plt.ylabel('Avg. Uncertainty ($)');
        plt.title('Uncertainty Relative to CPS')
        plt.grid(color='k', alpha=0.3)
        plt.xticks(results['cps'], results['cps'])
        plt.legend(prop={'size':10})
        plt.show();
